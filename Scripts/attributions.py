...
class LIGAttributions(Attributions):
    def __init__(
        ...
        if self.token_type_ids is not None and self.position_ids is not None:
            ...
            #print("the attributions self input id is: " + str(self.input_ids.shape[0]))
            view_pos1 = 4*self.input_ids.shape[0]
            #self.input_ids = self.input_ids.view(view_pos1,512)
            position_ids = np.full((view_pos1, 512), 0)
            for x in range(0,view_pos1):
                for y in range(0,len(position_ids[x])):
                    position_ids[x][y] = int(y)
            position_ids = torch.from_numpy(position_ids)
            #print(position_ids)
           # self.position_ids = position_ids#self.position_ids.view(view_pos1,512)
           # self.ref_input_ids = self.ref_input_ids.view(view_pos1,512)
           # self.ref_position_ids = self.ref_position_ids.view(view_pos1,512)
           # print(self.input_ids.shape)
           # print(self.position_ids.shape)
           # print(self.ref_input_ids)
           # print(self.ref_position_ids)
            self._attributions, self.delta = self.lig.attribute(
    ...
    def word_attributions(self) -> list:
        ...
        if len(self.attributions_sum) >= 1:
            self.attributions_sum = self.attributions_sum.view(1,2048)
            self.attributions_sum = self.attributions_sum.cpu().data.numpy()
            for i, (word, attribution) in enumerate(
                zip(self.tokens, self.attributions_sum[0])
            ):
                wa.append((word, float(attribution)))
          #  print(wa)
            return wa
